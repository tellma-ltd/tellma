...
export class CurrenciesMasterComponent extends MasterBaseComponent {
  ...
  public onDeactivate = (ids: (number | string)[]): Observable<any> => {
    const obs$ = this.currenciesApi.deactivate(ids, { returnEntities: true, expand: this.expand }).pipe(
      tap(res => addToWorkspace(res, this.workspace))
    );
  ...
  public canActivateDeactivateItem = (_: (number | string)[]) => this.ws.canDo('currencies', 'IsActive', null);
