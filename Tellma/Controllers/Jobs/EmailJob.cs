                ...
                try // To make sure the background service keeps running
                {
                    ...
                    foreach (var emailsOfTenant in emails.GroupBy(e => e.TenantId))
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            stateUpdates = emailsOfTenant.Select(e => new IdStateErrorTimestamp { Id = e.EmailId, State = EmailState.DeliveryFailed, Timestamp = DateTimeOffset.Now, Error = ex.Message });
