...
{
    /// <summary>
    /// Implementation of <see cref="IDataExtractor"/> for Excel files
    /// </summary>
    public class ExcelExtractor : IDataExtractor
    {
        /// <summary>
        /// Extract raw data from a XLSX file into an <see cref="IEnumerable{T}"/> of string arrays
        /// </summary>
        ...
        public IEnumerable<string[]> Extract(Stream xlsxStream)
        {
            ...
            while (reader.Read())
            {
                ...
                if (dataRow.Count > 0)
                {
                    ...
                    throw new BadRequestException("Excel file is corrupted, multiple row elements have the same reference");
                }
                ...
                foreach (var cell in cells)
                {
                    ...
                    if (cell.DataType != null && cell.DataType == CellValues.SharedString)
                    {
                        ...
                        {
                            throw new BadRequestException("Excel file is corrupted, a shared string value could not be interpreted as an integer index");
                        }

                        if (stringIndex >= sharedStrings.Length)
                        {
                            throw new BadRequestException("Excel file is corrupted, a shared string index is outside the range of the shared string table");
                        }
            ...
            foreach (var dataRow in data)
            {
                ...
                foreach (var dataCell in dataRow)
                {
                    ...
                    {
                        throw new BadRequestException("Excel file is corrupted, multiple cell elements have the same reference");
                    }
                    else
                    {
