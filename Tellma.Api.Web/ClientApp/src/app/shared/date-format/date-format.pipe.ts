...
export class DateFormatPipe implements PipeTransform, OnDestroy {
  ...
  private _granularity: DateGranularity;
  ...
  constructor(private workspace: WorkspaceService, private translate: TranslateService) {
    ...
    this._subscriptions.add(this.translate.onLangChange.subscribe(() => {
      this.markForCheck(); // In case a month name was used
    }));
  ...
  transform(isoDate: string, calendar?: Calendar, granularity?: DateGranularity): string {
    if (this._markForCheck || this._isoDate !== isoDate || this._calendar !== calendar || this._granularity !== granularity) {
      ...
      // Format
      this._formattedValue = dateFormat(isoDate, this.workspace, this.translate, calendar, granularity);
    }
