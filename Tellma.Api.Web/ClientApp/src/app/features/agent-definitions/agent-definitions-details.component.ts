...
import { AgentDefinitionForSave, metadata_AgentDefinition, AgentDefinition } from '~/app/data/entities/agent-definition';
...
import { AgentDefinitionReportDefinition } from '~/app/data/entities/agent-definition-report-definition';
...
export class AgentDefinitionsDetailsComponent extends DetailsBaseComponent {
  ...
  private allVisibilityProps(): string[] {
    const props = metadata_AgentDefinition(this.workspace, this.translate).properties;
    const result = [];
  ...
  public savePreprocessing = (entity: AgentDefinition) => {
  ...
  public isInactive: (model: AgentDefinition) => string = (_: AgentDefinition) => null;
  ...
  public sectionErrors(section: string, model: AgentDefinition) {
    if (section === 'Title') {
      return (!!model.serverErrors && (
  ...
  public onDefinitionChange(model: AgentDefinition, prop?: string) {
    if (!!prop) {
  ...
  private _currentModel: AgentDefinition;
  private _currentModelModified = false;
  private _getForClientResult: AgentDefinitionForClient;

  public getForClient(model: AgentDefinition): AgentDefinitionForClient {
    if (!model) {
      return null;
    }
    ...
    if (this._currentModel !== model || this._currentModelModified) {
      ...
      this._getForClientResult = { ...model } as AgentDefinitionForClient;
  ...
  public get allMainMenuSections(): SelectorChoice[] {
    const desc = metadata_AgentDefinition(this.workspace, this.translate).properties.MainMenuSection as ChoicePropDescriptor;
    return getChoices(desc);
  }

  public get allMainMenuIcons(): SelectorChoice[] {
    const desc = metadata_AgentDefinition(this.workspace, this.translate).properties.MainMenuIcon as ChoicePropDescriptor;
    return getChoices(desc);
  }


  public onIconClick(model: AgentDefinition, icon: SelectorChoice): void {
    model.MainMenuIcon = icon.value;
    this.onDefinitionChange(model, 'MainMenuSortKey');
  }

  // State Management
  public onMakeHidden = (model: AgentDefinition): void => {
    if (!!model && !!model.Id && model.State !== 'Hidden') {
      this.agentDefinitionsApi.updateState([model.Id], { state: 'Hidden', returnEntities: true }).pipe(
  ...
  public onMakeArchived = (model: AgentDefinition): void => {
    if (!!model && !!model.Id && model.State !== 'Archived') {
      this.agentDefinitionsApi.updateState([model.Id], { state: 'Archived', returnEntities: true }).pipe(
        tap(res => addToWorkspace(res, this.workspace))
      ).subscribe({ error: this.details.handleActionError });
    }
  }
  ...
  public hasStatePermission = (model: AgentDefinition) => this.ws.canDo('agent-definitions', 'State', model.Id);

  public stateTooltip = (model: AgentDefinition) => this.hasStatePermission(model) ? '' :
    this.translate.instant('Error_AccountDoesNotHaveSufficientPermissions')

  // Report Definitions

  public itemToEditHasChanged = false;
  public reportDefinitionToEdit: AgentDefinitionReportDefinition;

  public onItemToEditChange() {
    this.itemToEditHasChanged = true;
  }

  public onCreateReportDefinition(model: AgentDefinition) {
    const itemToEdit: AgentDefinitionReportDefinition = {};
  ...
  public onConfigureReportDefinition(index: number, model: AgentDefinition) {
    this.itemToEditHasChanged = false;
    const itemToEdit = { ...model.ReportDefinitions[index] } as AgentDefinitionReportDefinition;
  ...
  public onDeleteReportDefinition(index: number, model: AgentDefinition) {
  ...
  public onEditScript(scriptName: string, model: AgentDefinition) {
    // Prep
