...
namespace Tellma.Repository.Application
{
    ...
    {
        private readonly Func<CancellationToken, Task<string>> _connStringResolver;
        private readonly ILogger _logger;

        public ApplicationRepository(Func<CancellationToken, Task<string>> connStringResolver, ILogger<ApplicationRepository> logger)
        {
            _connStringResolver = connStringResolver;
            _logger = logger;
        }

        private Task<string> GetConnectionString(CancellationToken cancellation) => _connStringResolver(cancellation);
        ...
        public async Task Bla()
        {
            ...
            _logger.LogInformation(nameof(Bla), sw.ElapsedMilliseconds);
        }
