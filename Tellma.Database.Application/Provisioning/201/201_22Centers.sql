...
(120,12, N'Vehicles - Shared Expenses', N'ተሽከርካሪዎች - የጋራ ወጪዎች', N'120', N'SharedExpenseControl'),
...
(232,23, N'Site 2 - Export Grain Cleaning', N'ጣቢያ 2 - ላክ የቅንጣት ጽዳት', N'232', N'WorkInProgressExpendituresControl'),
(25,2, N'Export Shipments', N'ወደ ውጭ ይላኩ መርከቦች', N'25', N'Abstract'),
(251,25, N'Export Permit 1', N'የውጭ ንግድ ፈቃድ 1', N'251', N'CurrentInventoriesInTransitExpendituresControl'),
...
-- Declarations
DECLARE @106C_HeadOffice INT = (SELECT [Id] FROM dbo.[Centers] WHERE [CenterType] <> N'Abstract' AND [Name] = N'Head Office');
...
DECLARE @106C_AABuildingConstructionProject INT = (SELECT [Id] FROM dbo.[Centers] WHERE [CenterType] <> N'Abstract' AND [Name] = N'AA Building Construction Project');