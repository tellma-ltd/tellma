...
IF NOT EXISTS(SELECT * FROM dbo.[RelationDefinitions])
BEGIN
	IF @DB = N'100' -- ACME, USD, en/ar/zh playground
	BEGIN
		INSERT INTO @RelationDefinitions([Index],
...
--	
		...
		INSERT INTO @RelationDefinitions([Index],
		[Code],			[TitleSingular],[TitleSingular2],[TitlePlural],[TitlePlural2],	[JobVisibility], [RatesVisibility], [RatesLabel], [MainMenuIcon], [MainMenuSection], [MainMenuSortKey]) VALUES
...
EXEC [api].[RelationDefinitions__Save]
	...
	DECLARE @creditorsDef INT = (SELECT [Id] FROM dbo.[RelationDefinitions] WHERE [Code] = N'creditors');
	DECLARE @customersDef INT = (SELECT [Id] FROM dbo.[RelationDefinitions] WHERE [Code] = N'customers');
	DECLARE @debtorsDef INT = (SELECT [Id] FROM dbo.[RelationDefinitions] WHERE [Code] = N'debtors');
	DECLARE @partnersDef INT = (SELECT [Id] FROM dbo.[RelationDefinitions] WHERE [Code] = N'partners');
	DECLARE @suppliersDef INT = (SELECT [Id] FROM dbo.[RelationDefinitions] WHERE [Code] = N'suppliers');
	DECLARE @inventory_custodiansDef INT = (SELECT [Id] FROM dbo.[RelationDefinitions] WHERE [Code] = N'inventory-custodians');
	DECLARE @cash_custodiansDef INT = (SELECT [Id] FROM dbo.[RelationDefinitions] WHERE [Code] = N'cash-custodians');
	DECLARE @employeesDef INT = (SELECT [Id] FROM dbo.[RelationDefinitions] WHERE [Code] = N'employees');
