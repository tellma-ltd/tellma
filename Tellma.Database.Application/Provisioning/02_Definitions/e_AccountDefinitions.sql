...
INSERT INTO dbo.AccountDefinitions([Id],[ShowOCE],[MapFunction],
	[Code],						[Name]) VALUES
...
DECLARE @employee_bonusADef INT = (SELECT [Id] FROM dbo.AccountDefinitions WHERE [Code] = N'employee-bonus')
...
DECLARE @vat_payableADef INT = (SELECT [Id] FROM dbo.AccountDefinitions WHERE [Code] = N'vat-payable')
DECLARE @vat_receivableADef INT = (SELECT [Id] FROM dbo.AccountDefinitions WHERE [Code] = N'vat-receivable')
DECLARE @purchase_expenseADef INT = (SELECT [Id] FROM dbo.AccountDefinitions WHERE [Code] = N'purchase-expense')
DECLARE @document_controlADef INT = (SELECT [Id] FROM dbo.AccountDefinitions WHERE [Code] = N'document-control')
DECLARE @eitaxADef INT = (SELECT [Id] FROM dbo.AccountDefinitions WHERE [Code] = N'employee-income-tax')
DECLARE @estaxADef INT = (SELECT [Id] FROM dbo.AccountDefinitions WHERE [Code] = N'employee-stamp-tax')
