CREATE FUNCTION [bll].[ft_Employees_Period_EventFromModel_OffPaysheet__Generate]
(
	@FromDate DATE,
	@ToDate DATE,
	@ResourceId INT = NULL,
...
BEGIN
	...
	SELECT @Daily = [Id] FROM dbo.Units WHERE [Code] = N'd';

	SELECT @ContractLineDefinitionId = [Id] FROM dbo.LineDefinitions WHERE [Code] = N'ToEmployeeBenefitsExpenseFromAccruals.OP.M';
	SELECT @ContractAmendmentLineDefinitionId = ISNULL((SELECT [Id] FROM dbo.LineDefinitions WHERE [Code] = N'ToEmployeeBenefitsExpenseFromAccrualsAmended.OP.M'), 0);
	SELECT @ContractTerminationLineDefinitionId = ISNULL((SELECT [Id] FROM dbo.LineDefinitions WHERE [Code] = N'ToEmployeeBenefitsExpenseFromAccrualsTerminated.OP.M'), 0);
	...
	WHERE L.[State] = 2
	AND E.[DurationUnitId] IN(@Daily, @Monthly)
	...
	RunningTotals AS (
		...
		WHERE [MonetaryValue] <> 0 AND [Value] <> 0 -- MA:2026-02-01
	)
	INSERT INTO @T (
