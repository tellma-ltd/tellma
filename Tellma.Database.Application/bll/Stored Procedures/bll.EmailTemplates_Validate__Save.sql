CREATE PROCEDURE [bll].[EmailTemplates_Validate__Save]
	@Entities [EmailTemplateList] READONLY,
...
BEGIN
    ...
    FROM @Entities
    WHERE [Id] <> 0
	AND [Id] NOT IN (SELECT [Id] from [dbo].[EmailTemplates]);
	...
	SELECT DISTINCT TOP (@Top)
		...
		FE.Code
	FROM @Entities FE 
	JOIN [dbo].[EmailTemplates] BE ON FE.[Code] = BE.[Code]
