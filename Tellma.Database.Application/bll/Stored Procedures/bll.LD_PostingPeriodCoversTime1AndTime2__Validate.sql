...
FORMAT(FE.[Time1], 'de-de') AS [Time1],
FORMAT(FE.[Time2], 'de-de') AS [Time1]
FROM @Documents FD
...
AND (
	FE.[Time1] < dbo.fn_PeriodStart(FE.[DurationUnitId], FL.[PostingDate]) OR
	FE.[Time2] > dbo.fn_PeriodStart(FE.[DurationUnitId], FL.[PostingDate])
);
