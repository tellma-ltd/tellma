CREATE PROCEDURE [bll].[NotificationTemplates_Validate__Save]
...
BEGIN
    ...
    FROM @Entities
    WHERE [Id] <> 0
	AND [Id] NOT IN (SELECT [Id] from [dbo].[NotificationTemplates]);
	...
	SELECT DISTINCT TOP (@Top)
		...
		FE.Code
	FROM @Entities FE 
	JOIN [dbo].[NotificationTemplates] BE ON FE.[Code] = BE.[Code]
