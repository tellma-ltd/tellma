CREATE PROCEDURE [bll].[Reconciliations_Validate__Insert]
	@Entries IndexedIdList READONLY,
	@ExternalEntries IndexedIdList READONLY,
	@Top INT = 10
AS
SET NOCOUNT ON;
	DECLARE @ValidationErrors [dbo].[ValidationErrorList];
	-- Make sure account and custodies in books, with non zero values match with bank
	WITH T AS (
		SELECT E.[Index], E.[AccountId], E.[CustodyId], EX.[AccountId] AS EXAccountID, EX.CustodyId AS EXCustodyId
		FROM
		(
	...
	SELECT TOP(@Top)	
		...
		N'Error_Account0HasNoMatchInExternalEntries',
		dbo.fn_Localize(A.[Name], A.[Name2], A.[Name3]) AS [Account], NULL
	FROM T
	JOIN dbo.Accounts A ON T.[AccountId] = A.[Id]
	WHERE EXAccountId IS NULL
	UNION
	SELECT TOP(@Top)	
		'[' + CAST(T.[Index] AS NVARCHAR (255)) + ']',
		N'Error_Custody01HasNoMatchInExternalEntries',
