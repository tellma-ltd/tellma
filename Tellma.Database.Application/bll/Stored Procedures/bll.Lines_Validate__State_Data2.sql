...
BEGIN
	...
	SELECT
		L.[DocumentIndex], L.[Index] AS [LineIndex], E.[Index], BL.[PostingDate], BD.Code, E.ResourceId, E.[RelationId],
		SUM(BE.[Direction] * BE.[Quantity]) 
			OVER (Partition BY BE.[ResourceId], BE.[RelationId] ORDER BY BL.[PostingDate], BE.[Direction] DESC) AS RunningDailyTotal,
		SUM(BE.[Direction] * BE.[Quantity]) 
			OVER (Partition BY BE.[ResourceId], BE.[RelationId] ORDER BY MONTH(BL.[PostingDate]), BE.[Direction] DESC) AS RunningMonthlyTotal
		FROM (
