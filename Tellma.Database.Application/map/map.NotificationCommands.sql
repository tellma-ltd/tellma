...
RETURN (
	...
	(SELECT COUNT(*) FROM [dbo].[SmsMessages] WHERE [State] > 2 AND [CommandId] = [Q].[Id]) As [SmsSuccesses],
    (SELECT COUNT(*) FROM [dbo].[SmsMessages] WHERE [State] < 0 AND [CommandId] = [Q].[Id]) As [SmsErrors],
    (SELECT COUNT(*) FROM [dbo].[SmsMessages] WHERE [CommandId] = [Q].[Id]) As [SmsTotal]
