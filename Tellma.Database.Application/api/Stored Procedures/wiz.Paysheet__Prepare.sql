...
--9  Cr. 11% Pension Payable
	...
	SET
		...
		PE.[AccountId] = (
			...
			WHERE [IfrsTypeId] = R.[AssetTypeId]
			AND ([CenterId] IS NULL OR [CenterId] = PE.CenterId)
			AND ([RelationId] IS NULL OR [RelationId] = PE.[RelationId])
			AND ([ResourceId] IS NULL OR [ResourceId] = PE.[ResourceId])
			AND ([CurrencyId] IS NULL OR [CurrencyId] = PE.[CurrencyId])
		)
	FROM @ProcessedEntries PE
	JOIN dbo.AgentRates AR ON PE.NotedRelationId = AR.[Id] AND PE.ResourceId = AR.ResourceId
