...
(
	...
	[GrossSalaryInBasicCurrency]	DECIMAL (19, 6)
...
BEGIN
	...
	UPDATE EP
	SET EP.[GrossSalaryInBasicCurrency] = EP.[BasicSalary] + ISNULL(
		(
			SELECT SUM(converted_amount)
			FROM (
				SELECT bll.fn_ConvertCurrencies(EP.[AsOfDate], OB.[CurrencyId], EP.[CurrencyId], OB.[Benefit]) AS converted_amount
				FROM (
					SELECT E.[CurrencyId], SUM(E.[Direction] * E.[MonetaryValue]) AS [Benefit]
					FROM dbo.Entries E
					JOIN dbo.Lines L ON L.[Id] = E.[LineId]
					JOIN dbo.Accounts A ON A.[Id] = E.[AccountId]
					JOIN dbo.AccountTypes AC ON AC.[Id] = A.[AccountTypeId]
					JOIN dbo.Resources R ON R.[Id] = E.[ResourceId]
					WHERE E.[NotedAgentId] = EP.[EmployeeId]
					AND E.[Time1] <= EP.[AsOfDate]
					AND (E.[Time2] IS NULL OR E.[Time2] >= EP.[AsOfDate])
					AND L.[State] = 2
					AND AC.[Concept] = N'WagesAndSalaries'
					AND R.[UnitId] = @MonthUnitId
					AND E.[ResourceId] <> @BasicSalaryRS
