...
BEGIN
	...
	(
		...
		USING (
			SELECT 
				...
				[MemoIsCommon],
				[DebitRelationId],
				[DebitRelationIsCommon],
				[CreditRelationId],
				[CreditRelationIsCommon],
				[NotedRelationId],
		...
		WHEN MATCHED THEN
			UPDATE SET
				...
				t.[DebitRelationId]			= s.[DebitRelationId],
				t.[DebitRelationIsCommon]	= s.[DebitRelationIsCommon],
				t.[CreditRelationId]		= s.[CreditRelationId],
				t.[CreditRelationIsCommon]	= s.[CreditRelationIsCommon],
		...
		WHEN NOT MATCHED THEN
			...
			VALUES (
				...
				s.[MemoIsCommon],
				s.[DebitRelationId],
				s.[DebitRelationIsCommon],
				s.[CreditRelationId],
				s.[CreditRelationIsCommon],
				s.[NotedRelationId],
	...
	USING (
		SELECT
			...
			E.[RelationId], E.[ContractId], E.[ResourceId], E.[CenterId],
	...
	WHEN NOT MATCHED THEN
		INSERT ([LineId], [Index], [Direction], [AccountId], [CurrencyId],
			[RelationId], [ContractId], [ResourceId], [CenterId],
		...
		VALUES (s.[LineId], s.[Index], s.[Direction], s.[AccountId], s.[CurrencyId],
			s.[RelationId], s.[ContractId], s.[ResourceId], s.[CenterId],
