...
--(7,8,	N'Entries', N'MonetaryValue',		1,	N'Depreciation',N'الإهلاك',		1,0,0);
	...
	WHERE ResourceAssignment <> N'N'
	AND [Node].IsDescendantOf(@PPENode) = 1; -- select * from entrytypes select * from centers

	INSERT INTO @WideLines([Index], DefinitionId,
			[DocumentIndex],ResourceId1,Quantity1,	UnitId1, EntryTypeId0,			Time11,	Time21,
			CurrencyId1,	CenterId0)
	SELECT	ROW_NUMBER() OVER(ORDER BY [Id]) - 1, @LineDefinitionId,
			@DocumentIndex, [Id],		@Quantity,	@UnitId, [ExpenseEntryTypeId],	@Time1,	@Time2, 
			[CurrencyId],	[CenterId]
	FROM dbo.Resources
	WHERE [AssetTypeId] IN (SELECT [Id] FROM @PPETypeIds);
