...
namespace Tellma.Api
{
    ...
    public class MetadataProvider
    {
        ...
        public async Task<TypeMetadata> GetMetadata2(int? tenantId, Type entityType, int? definitionId, string settingsVersion, string definitionsVersion)
        {
        ...
        private TypeMetadata GetMetadataImpl(int? tenantId, Type entityType, int? definitionId, SettingsForClient settings, DefinitionsForClient defs)
        {
            // Prepare the cache key
            var cacheKey = new CacheKey
            {
            ...
            var result = _cache.GetOrAdd(cacheKey, (cacheKey) =>
            {
                ...
                foreach (var propDesc in typeDesc.Properties)
                {
                    ...
                    if (propDesc is CollectionPropertyDescriptor collPropDesc)
                    {
                        ...
                        Type collectionType = propInfo.PropertyType.GetGenericArguments().SingleOrDefault();
                        TypeMetadata getCollectionTypeMetadata() => GetMetadataImpl(tenantId, collectionType, defOverride?.DefinitionId, settings, defs);
                    ...
                    else if (propDesc is NavigationPropertyDescriptor navPropDesc)
                    {
                        ...
                        TypeMetadata getTypeMetadata() => GetMetadataImpl(tenantId, propInfo.PropertyType, defOverride?.DefinitionId, settings, defs);
            ...
            if (result.Definitions != defs || result.Settings != settings)
            {
                // If the metadata are based on the wrong settings or definitions, clear and try again
                _cache.TryRemove(cacheKey, out result);
                return GetMetadataImpl(tenantId, entityType, definitionId, settings, defs);
            }
            else
            {
                return result.Metadata;
