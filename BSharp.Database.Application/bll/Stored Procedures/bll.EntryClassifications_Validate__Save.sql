CREATE PROCEDURE [bll].[EntryClassifications_Validate__Save]
...
SET NOCOUNT ON;
    ...
    FROM @Entities
    WHERE Id IN (SELECT Id from [dbo].[EntryClassifications] WHERE IsActive = 0)
	OPTION(HASH JOIN);
    ...
    FROM @Entities
    WHERE Id <> 0
	AND Id NOT IN (SELECT Id from [dbo].[EntryClassifications])
	OPTION(HASH JOIN);
	...
	FROM @Entities FE 
	JOIN [dbo].[EntryClassifications] BE ON FE.Code = BE.Code
	...
	SELECT 
		...
		FE.[Name]
	FROM @Entities FE 
	JOIN [dbo].[EntryClassifications] BE ON FE.[Name] = BE.[Name]
	WHERE (FE.Id <> BE.Id)
	OPTION(HASH JOIN);
	...
	SELECT
		...
		FE.[Name2]
	FROM @Entities FE 
	JOIN [dbo].[EntryClassifications] BE ON FE.[Name2] = BE.[Name2]
	WHERE (FE.Id <> BE.Id)
	OPTION(HASH JOIN);
	...
	SELECT
		...
		FE.[Name3]
	FROM @Entities FE 
	JOIN [dbo].[EntryClassifications] BE ON FE.[Name3] = BE.[Name3]
	WHERE (FE.Id <> BE.Id)
	OPTION(HASH JOIN);
