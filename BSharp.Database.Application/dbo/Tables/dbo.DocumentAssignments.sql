...
-- If it is in (Active) state, it will be automatically assigned to the person who moved it to (Active)
	[AssigneeId]	INT					NOT NULL CONSTRAINT [FK_DocumentAssignments__AssigneeId] FOREIGN KEY ([AssigneeId]) REFERENCES [dbo].[Users] ([Id]),
-- When moved to (Active) state, the comment is automatically To be completed in the user primary language
	[Comment]		NVARCHAR (1024),
	[CreatedAt]		DATETIMEOFFSET(7)	NOT NULL DEFAULT SYSDATETIMEOFFSET(),
	[CreatedById]	INT					NOT NULL DEFAULT CONVERT(INT, SESSION_CONTEXT(N'UserId')) CONSTRAINT [FK_DocumentAssignments__CreatedById] FOREIGN KEY ([CreatedById]) REFERENCES [dbo].[Users] ([Id]),
