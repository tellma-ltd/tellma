...
-- Major properties: NULL means it is not defined.
	[AccountTypeId]					NVARCHAR (50)		NOT NULL -- CONSTRAINT [FK_Accounts__AccountTypeId] 
...
--	b) if the type itself is not null, then it is to be defined in entries.
	[AgentId]						INT					--CONSTRAINT [FK_Accounts__AgentId] 
														REFERENCES [dbo].[Agents] ([Id]),
	[ResourceId]					INT					--CONSTRAINT [FK_Accounts__ResourceId] 
														REFERENCES [dbo].[Resources] ([Id]),
	[ResponsibilityCenterId]		INT					--CONSTRAINT [FK_Accounts__ResponsibilityCenterId]
														REFERENCES [dbo].[ResponsibilityCenters] ([Id]) ON DELETE CASCADE,
...
-- Entry Property
	...
	[CreatedById]					INT					NOT NULL DEFAULT CONVERT(INT, SESSION_CONTEXT(N'UserId')) -- CONSTRAINT [FK_Accounts__CreatedById] FOREIGN KEY ([CreatedById]) 
														REFERENCES [dbo].[Users] ([Id]),
	[ModifiedAt]					DATETIMEOFFSET(7)	NOT NULL DEFAULT SYSDATETIMEOFFSET(),
	[ModifiedById]					INT					NOT NULL DEFAULT CONVERT(INT, SESSION_CONTEXT(N'UserId')) -- CONSTRAINT [FK_Accounts__ModifiedById]  FOREIGN KEY ([ModifiedById]) 
														REFERENCES [dbo].[Users] ([Id])
);
GO
